...
Flight::group("/api/auth/login", function () {
    ...
    Flight::route("POST ", function () {
        ...
        if (is_array($validateResponse) && isset($validateResponse['token'])) {
            ...
            Flight::json(["message" => "Usuario autenticado", "token" => $validateResponse['token'], "role" => $validateResponse['role'], "urlImage" => $validateResponse['urlImage'] ?? null, "Nombres" => $validateResponse["Nombres"], "Apellidos" => $validateResponse["Apellidos"]], 200);
        } else {
